...
the documented ``resolve`` lookup order (clickup_email, then clockify_email, then
canonical_name).
"""
...
def _mapping(canonical_name: str, clickup_email: str, clockify_email: str) -> EmployeeMapping:
...
def test_duplicate_email_to_two_names_raises_value_error(shared_field: str) -> None:
    ...
    first = _mapping(
        "Alice Silva",
        clickup_email=shared if shared_field == "clickup_email" else "alice.jira@example.com",
        clockify_email=shared if shared_field == "clockify_email" else "alice.clockify@example.com",
    )
    second = _mapping(
        "Bob Souza",
        clickup_email=shared if shared_field == "clickup_email" else "bob.jira@example.com",
...
def test_resolve_matches_clickup_email(employee_registry: EmployeeRegistry) -> None:
    """A ClickUp email resolves to its canonical name."""
...
def test_resolve_prefers_clickup_email_over_canonical_name() -> None:
    """When a value is both a ClickUp email and another person's name, ClickUp wins.

    ``resolve`` tries the ClickUp-email index before scanning canonical names, so
    a string that happens to be both must resolve through the email index.
    """
    ...
    # "ann@example.com" is Ann's clickup_email and also, coincidentally, her
    # canonical name; the clickup-email branch must answer first.
